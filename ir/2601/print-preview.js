...
function applyContentZoom(idx, zoomPercent) {
  ...
  // gp-nda-row가 있는지 확인
  const hasGpNdaRow = clone.querySelector('.gp-nda-row');

  // Apply new zoom with !important to override CSS
  contentSelectors.forEach(selector => {
    // gp-nda-row가 있으면 하위 요소는 스킵 (중복 줌 방지)
    if (hasGpNdaRow && (selector === '.gp-top' || selector === '.gp-countries')) {
      return;
    }
...
function applyContentAlignmentToSlide(clone, alignV, alignH) {
  ...
  switch (alignV) {
    case 'top':
      // 상단 정렬: 타이틀 아래에 기본 여백 (타이틀 위 패딩과 동일하게 30px)
      firstContentElement.style.setProperty('margin-top', '30px', 'important');
      break;
    case 'middle':
      // 가운데 정렬: 타이틀 바로 아래(여백 없음) ~ 프레임 하단 기준 가운데
